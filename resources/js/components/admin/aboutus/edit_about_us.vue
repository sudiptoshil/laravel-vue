...
<div class="widget-body">
    <!-- BEGIN FORM-->
<form action="#" class="form-horizontal" @submit.prevent="updateabout()">
...
<script>
  ...
  created(){
    axios.get(`/edit-aboutus/${this.$route.params.id}`)
    .then((response)=>{
  ...
  methods:{
  	updateabout(){
  		this.form.post(`/update-about-us/${this.$route.params.id}`)
  		.then((response)=>{
    ...
    updateimage(){
      let img = this.form.about_us_image
      if (img.length >100 ) {
        return this.form.about_us_image
      }
      else{
        return `uploadimage/${this.form.about_us_image}`;
      }
    }
  }
	}
